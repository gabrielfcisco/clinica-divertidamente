<template>
    <AuthenticatedLayout>
      ...
      <div class="bg-white p-8 rounded-lg shadow-md w-full max-w-md mx-auto mt-6">
        <h2 class="text-2xl font-bold mb-6 text-center">Encaminhamento Médico</h2>
        <div class="space-y-4">
          <div>
          ...
          <div>
            <label class="block text-gray-700">Estou encaminhando a/o paciente</label>
          ...
          <div>
            <label class="block text-gray-700">Para o Doutor(a)</label>
            <input type="text" v-model="referredDoctor" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
          </div>
          <div>
            <label class="block text-gray-700">Pelos motivo de</label>
  ...
  // Add title
  const title = 'Encaminhamento Médico';
  page.drawText(title, {
    x: 50,
    y: 370,
    size: 18,
  ...
  // Add text content
  const textContent = [
    `Eu Doutor(a) ${doctorName.value} Estou encaminhando a/o paciente ${patientName.value}`,
    `Para o Doutor(a) ${referredDoctor.value} Pelos motivo de:`,
    `${reason.value}`,
  ];
